...
2. **캐릭터 연결**: 프롬프트에 `@쿠앤크` 태그 (등록된 캐릭터 자산 호출 = 레퍼런스
   ...
   주의: Flow의 @캐릭터 기능은 **Fast에서만 지원될 수 있음** (Flow 제품 기능이라
   모델별 단계 지원 — UI 안내 기준). 모델 자체의 레퍼런스 일관성(Ingredients)은
   Fast·Quality 모두 지원이므로, Quality가 필요하면 @태그 대신 **+ 버튼으로
   캐릭터 시트 이미지를 직접 첨부**하면 됨. 파일럿 8편은 Fast 통일 권장
   (자막이 화면을 덮는 릴스 특성상 화질 체감차 작음, 비용 1/4, 리테이크 여유).
   **실측 확인(2026-08-28): Quality는 확장(Extend)도 미지원** → 캐릭터·확장·
   오디오가 모두 되는 건 Fast뿐이므로 **Fast가 표준 파이프라인**. Quality는 추후
   성과 편 리마스터 시 확장 없이 클립별 단독 재생성(+캐릭터 시트 첨부)으로만 사용
3. **클립 1 생성** (§4 프롬프트, 캐릭터 묘사 대신 @쿠앤크) → 2안 비교, 한국어
   발음 어색하면 같은 프롬프트 재생성 2~3회로 톤 확정 (전 과정 Fast)
4. **클립 2·3 — 확장(Extend) 정확한 경로** (확장 메뉴는 씬빌더 안에 있음):
   클립에 마우스 → 더보기(⋮) → **"장면에 추가(Add to Scene)"** → 씬빌더로 자동
   이동 → 타임라인에서 클립 끝의 **+ 아이콘** → **"확장(Extend)"** → "다음에 어떤
   일이 일어날까요?" 입력창에 이어질 내용만 입력(캐릭터·배경 묘사 불필요 — 마지막
   프레임에서 그대로 이어짐).
   제약: Veo 생성 클립만 확장 가능, 확장 클립엔 삽입/제거/카메라 편집 불가 →
